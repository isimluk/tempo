  ...
  class PlanInput
    ...
    def self.attribute_map
      {
        ...
        :'planned_seconds_per_day' => :'plannedSecondsPerDay',
    ...
    def self.openapi_types
      {
        ...
        :'plan_approval' => :'PlanApprovalInput',
        :'plan_item_id' => :'String',
        :'plan_item_type' => :'String',
        :'planned_seconds_per_day' => :'Integer',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PlanInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PlanInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'assignee_type')
        self.assignee_type = attributes[:'assignee_type']
      end
      ...
      if attributes.key?(:'include_non_working_days')
        self.include_non_working_days = attributes[:'include_non_working_days']
      end

      if attributes.key?(:'plan_approval')
        self.plan_approval = attributes[:'plan_approval']
      end

      if attributes.key?(:'plan_item_id')
        self.plan_item_id = attributes[:'plan_item_id']
      end

      if attributes.key?(:'plan_item_type')
        self.plan_item_type = attributes[:'plan_item_type']
      end

      if attributes.key?(:'planned_seconds_per_day')
        self.planned_seconds_per_day = attributes[:'planned_seconds_per_day']
      end

      if attributes.key?(:'recurrence_end_date')
        self.recurrence_end_date = attributes[:'recurrence_end_date']
      end
    ...
    def list_invalid_properties
      ...
      if @assignee_type.nil?
        invalid_properties.push('invalid value for "assignee_type", assignee_type cannot be nil.')
      end
      ...
      if @plan_item_id.nil?
        invalid_properties.push('invalid value for "plan_item_id", plan_item_id cannot be nil.')
      end

      if @plan_item_type.nil?
        invalid_properties.push('invalid value for "plan_item_type", plan_item_type cannot be nil.')
      end

      if @planned_seconds_per_day.nil?
        invalid_properties.push('invalid value for "planned_seconds_per_day", planned_seconds_per_day cannot be nil.')
      end

      if @planned_seconds_per_day < 1
        invalid_properties.push('invalid value for "planned_seconds_per_day", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @assignee_type.nil?
      assignee_type_validator = EnumAttributeValidator.new('String', ["USER", "GENERIC"])
      return false unless assignee_type_validator.valid?(@assignee_type)
      return false if @end_date.nil?
      return false if @plan_item_id.nil?
      return false if @plan_item_type.nil?
      plan_item_type_validator = EnumAttributeValidator.new('String', ["ISSUE", "PROJECT"])
      return false unless plan_item_type_validator.valid?(@plan_item_type)
      return false if @planned_seconds_per_day.nil?
      return false if @planned_seconds_per_day < 1
      rule_validator = EnumAttributeValidator.new('String', ["NEVER", "WEEKLY", "BI_WEEKLY", "MONTHLY"])
    ...
    # @param [Object] assignee_type Object to be assigned
    def assignee_type=(assignee_type)
      validator = EnumAttributeValidator.new('String', ["USER", "GENERIC"])
      unless validator.valid?(assignee_type)
        fail ArgumentError, "invalid value for \"assignee_type\", must be one of #{validator.allowable_values}."
      end
      @assignee_type = assignee_type
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] plan_item_type Object to be assigned
    def plan_item_type=(plan_item_type)
      validator = EnumAttributeValidator.new('String', ["ISSUE", "PROJECT"])
      unless validator.valid?(plan_item_type)
        fail ArgumentError, "invalid value for \"plan_item_type\", must be one of #{validator.allowable_values}."
      end
      @plan_item_type = plan_item_type
    end

    # Custom attribute writer method with validation
    # @param [Object] planned_seconds_per_day Value to be assigned
    def planned_seconds_per_day=(planned_seconds_per_day)
      if planned_seconds_per_day.nil?
        fail ArgumentError, 'planned_seconds_per_day cannot be nil'
      end

      if planned_seconds_per_day < 1
        fail ArgumentError, 'invalid value for "planned_seconds_per_day", must be greater than or equal to 1.'
      end

      @planned_seconds_per_day = planned_seconds_per_day
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] rule Object to be assigned
    def rule=(rule)
      validator = EnumAttributeValidator.new('String', ["NEVER", "WEEKLY", "BI_WEEKLY", "MONTHLY"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          end_date == o.end_date &&
          include_non_working_days == o.include_non_working_days &&
          plan_approval == o.plan_approval &&
          plan_item_id == o.plan_item_id &&
          plan_item_type == o.plan_item_type &&
          planned_seconds_per_day == o.planned_seconds_per_day &&
          recurrence_end_date == o.recurrence_end_date &&
          rule == o.rule &&
    ...
    def hash
      [assignee_id, assignee_type, description, end_date, include_non_working_days, plan_approval, plan_item_id, plan_item_type, planned_seconds_per_day, recurrence_end_date, rule, start_date].hash
    end
