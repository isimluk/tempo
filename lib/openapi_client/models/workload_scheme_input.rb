  ...
  class WorkloadSchemeInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::WorkloadSchemeInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::WorkloadSchemeInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@days.nil? && @days.length > 7
        invalid_properties.push('invalid value for "days", number of items must be less than or equal to 7.')
      end

      if !@days.nil? && @days.length < 7
        invalid_properties.push('invalid value for "days", number of items must be greater than or equal to 7.')
      end
    ...
    def valid?
      return false if !@days.nil? && @days.length > 7
      return false if !@days.nil? && @days.length < 7
    ...
    def days=(days)
      if !days.nil? && days.length > 7
        fail ArgumentError, 'invalid value for "days", number of items must be less than or equal to 7.'
      end

      if !days.nil? && days.length < 7
        fail ArgumentError, 'invalid value for "days", number of items must be greater than or equal to 7.'
      end
