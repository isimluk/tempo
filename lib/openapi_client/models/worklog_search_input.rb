  ...
  class WorklogSearchInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::WorklogSearchInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::WorklogSearchInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'author_ids')
        if (value = attributes[:'author_ids']).is_a?(Array)
          self.author_ids = value
        end
      end
      ...
      if attributes.key?(:'issue_ids')
        if (value = attributes[:'issue_ids']).is_a?(Array)
          self.issue_ids = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          author_ids == o.author_ids &&
          from == o.from &&
          issue_ids == o.issue_ids &&
    ...
    def hash
      [author_ids, from, issue_ids, project_ids, to, updated_from].hash
    end
