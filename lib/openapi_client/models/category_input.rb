  ...
  class CategoryInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CategoryInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CategoryInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_name_validator = EnumAttributeValidator.new('String', ["BILLABLE", "CAPITALIZED", "INTERNAL", "OPERATIONAL"])
      return false unless type_name_validator.valid?(@type_name)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] type_name Object to be assigned
    def type_name=(type_name)
      validator = EnumAttributeValidator.new('String', ["BILLABLE", "CAPITALIZED", "INTERNAL", "OPERATIONAL"])
      unless validator.valid?(type_name)
        fail ArgumentError, "invalid value for \"type_name\", must be one of #{validator.allowable_values}."
      end
      @type_name = type_name
    end
