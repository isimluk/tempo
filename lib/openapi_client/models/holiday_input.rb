  ...
  class HolidayInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::HolidayInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::HolidayInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @duration_seconds < 1
        invalid_properties.push('invalid value for "duration_seconds", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @duration_seconds < 1
      return false if @name.nil?
      return false if @type.nil?
      type_validator = EnumAttributeValidator.new('String', ["FIXED", "FLOATING"])
    ...
    def duration_seconds=(duration_seconds)
      ...
      if duration_seconds < 1
        fail ArgumentError, 'invalid value for "duration_seconds", must be greater than or equal to 1.'
      end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["FIXED", "FLOATING"])
    ...
    def hash
      [date, description, duration_seconds, name, type].hash
    end
