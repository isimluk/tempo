  ...
  class TeamLinkInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TeamLinkInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TeamLinkInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @scope_id < 1
        invalid_properties.push('invalid value for "scope_id", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @scope_id < 1
      return false if @scope_type.nil?
      scope_type_validator = EnumAttributeValidator.new('String', ["PROJECT"])
      return false unless scope_type_validator.valid?(@scope_type)
      return false if @team_id.nil?
      return false if @team_id < 1
      true
    end
    ...
    def scope_id=(scope_id)
      ...
      if scope_id < 1
        fail ArgumentError, 'invalid value for "scope_id", must be greater than or equal to 1.'
      end
    ...
    def scope_type=(scope_type)
      ...
      unless validator.valid?(scope_type)
        fail ArgumentError, "invalid value for \"scope_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [scope_id, scope_type, team_id].hash
    end
