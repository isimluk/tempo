...
module OpenapiClient
  class GenericResource
    ...
    def self.openapi_types
      {
        :'created_at' => :'String',
        :'created_by' => :'UserRefBean',
        :'id' => :'Integer',
        :'name' => :'String',
        :'_self' => :'String',
        :'updated_at' => :'String',
        :'updated_by' => :'UserRefBean'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GenericResource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GenericResource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [created_at, created_by, id, name, _self, updated_at, updated_by].hash
    end
