...
module OpenapiClient
  class HolidaySchemeMembersInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::HolidaySchemeMembersInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::HolidaySchemeMembersInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@account_ids.nil? && @account_ids.length > 2147483647
        invalid_properties.push('invalid value for "account_ids", number of items must be less than or equal to 2147483647.')
      end

      if !@account_ids.nil? && @account_ids.length < 1
        invalid_properties.push('invalid value for "account_ids", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      return false if !@account_ids.nil? && @account_ids.length > 2147483647
      return false if !@account_ids.nil? && @account_ids.length < 1
      true
    end
    ...
    def account_ids=(account_ids)
      if !account_ids.nil? && account_ids.length > 2147483647
        fail ArgumentError, 'invalid value for "account_ids", number of items must be less than or equal to 2147483647.'
      end

      if !account_ids.nil? && account_ids.length < 1
        fail ArgumentError, 'invalid value for "account_ids", number of items must be greater than or equal to 1.'
      end
