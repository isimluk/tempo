  ...
  class TeamInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TeamInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TeamInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@lead_account_id.nil? && @lead_account_id !~ pattern
        invalid_properties.push("invalid value for \"lead_account_id\", must conform to the pattern #{pattern}.")
      end
      ...
      if !@program_id.nil? && @program_id < 1
        invalid_properties.push('invalid value for "program_id", must be greater than or equal to 1.')
      end
    ...
    def valid?
      return false if !@lead_account_id.nil? && @lead_account_id !~ Regexp.new(/^[a-zA-Z0-9-:_.]*$/)
      return false if @name.nil?
      return false if !@program_id.nil? && @program_id < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] lead_account_id Value to be assigned
    def lead_account_id=(lead_account_id)
      pattern = Regexp.new(/^[a-zA-Z0-9-:_.]*$/)
      if !lead_account_id.nil? && lead_account_id !~ pattern
        fail ArgumentError, "invalid value for \"lead_account_id\", must conform to the pattern #{pattern}."
      end
    ...
    def program_id=(program_id)
      if !program_id.nil? && program_id < 1
        fail ArgumentError, 'invalid value for "program_id", must be greater than or equal to 1.'
      end
    ...
    def hash
      [lead_account_id, name, program_id, public, summary].hash
    end
