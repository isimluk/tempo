  ...
  class TimesheetApprovalInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TimesheetApprovalInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TimesheetApprovalInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'reviewer_account_id')
        self.reviewer_account_id = attributes[:'reviewer_account_id']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          comment == o.comment &&
          reviewer_account_id == o.reviewer_account_id
    end
    ...
    def hash
      [comment, reviewer_account_id].hash
    end
