...
module OpenapiClient
  class TempoWorklogIdList
    ...
    def self.openapi_types
      {
        :'tempo_worklog_ids' => :'Array<Integer>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TempoWorklogIdList` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TempoWorklogIdList`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'tempo_worklog_ids')
        if (value = attributes[:'tempo_worklog_ids']).is_a?(Array)
          self.tempo_worklog_ids = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @tempo_worklog_ids.nil?
        invalid_properties.push('invalid value for "tempo_worklog_ids", tempo_worklog_ids cannot be nil.')
      end

      if @tempo_worklog_ids.length > 2147483647
        invalid_properties.push('invalid value for "tempo_worklog_ids", number of items must be less than or equal to 2147483647.')
      end

      if @tempo_worklog_ids.length < 1
        invalid_properties.push('invalid value for "tempo_worklog_ids", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      return false if @tempo_worklog_ids.nil?
      return false if @tempo_worklog_ids.length > 2147483647
      return false if @tempo_worklog_ids.length < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] tempo_worklog_ids Value to be assigned
    def tempo_worklog_ids=(tempo_worklog_ids)
      if tempo_worklog_ids.nil?
        fail ArgumentError, 'tempo_worklog_ids cannot be nil'
      end

      if tempo_worklog_ids.length > 2147483647
        fail ArgumentError, 'invalid value for "tempo_worklog_ids", number of items must be less than or equal to 2147483647.'
      end

      if tempo_worklog_ids.length < 1
        fail ArgumentError, 'invalid value for "tempo_worklog_ids", number of items must be greater than or equal to 1.'
      end

      @tempo_worklog_ids = tempo_worklog_ids
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          tempo_worklog_ids == o.tempo_worklog_ids
    end
