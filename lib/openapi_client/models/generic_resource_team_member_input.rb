...
module OpenapiClient
  class GenericResourceTeamMemberInput
    # The id of the `Generic Resource` itself
    attr_accessor :generic_resource_id

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'generic_resource_id' => :'genericResourceId'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GenericResourceTeamMemberInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GenericResourceTeamMemberInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'generic_resource_id')
        self.generic_resource_id = attributes[:'generic_resource_id']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @generic_resource_id.nil?
        invalid_properties.push('invalid value for "generic_resource_id", generic_resource_id cannot be nil.')
      end
    ...
    def valid?
      return false if @generic_resource_id.nil?
      true
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          generic_resource_id == o.generic_resource_id
    end
