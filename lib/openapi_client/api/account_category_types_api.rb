...
module OpenapiClient
  class AccountCategoryTypesApi
    ...
    # Retrieves a list of all Category Types
    # @param [Hash] opts the optional parameters
    # @return [Array<(CategoryTypeResults, Integer, Hash)>] CategoryTypeResults data, response status code and response headers
    def get_types_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: AccountCategoryTypesApi.get_types ...'
      end
      # resource path
      local_var_path = '/account-category-types'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'CategoryTypeResults'

      # auth_names
      auth_names = opts[:debug_auth_names] || []

      new_options = opts.merge(
        :operation => :"AccountCategoryTypesApi.get_types",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AccountCategoryTypesApi#get_types\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
