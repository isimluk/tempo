...
module OpenapiClient
  class PeriodsApi
    ...
    # @return [TimesheetApprovalPeriodsBean]
    def get_periods(from, to, opts = {})
      data, _status_code, _headers = get_periods_with_http_info(from, to, opts)
      data
    end
    ...
    # @param to [String] The end date in format yyyy-mm-dd
    # @param [Hash] opts the optional parameters
    # @return [Array<(TimesheetApprovalPeriodsBean, Integer, Hash)>] TimesheetApprovalPeriodsBean data, response status code and response headers
    def get_periods_with_http_info(from, to, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: PeriodsApi.get_periods ...'
      end
      # verify the required parameter 'from' is set
      if @api_client.config.client_side_validation && from.nil?
        fail ArgumentError, "Missing the required parameter 'from' when calling PeriodsApi.get_periods"
      end
      # verify the required parameter 'to' is set
      if @api_client.config.client_side_validation && to.nil?
        fail ArgumentError, "Missing the required parameter 'to' when calling PeriodsApi.get_periods"
      end
      # resource path
      local_var_path = '/periods'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'TimesheetApprovalPeriodsBean'

      # auth_names
      auth_names = opts[:debug_auth_names] || []

      new_options = opts.merge(
        :operation => :"PeriodsApi.get_periods",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PeriodsApi#get_periods\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
